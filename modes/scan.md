...
> **Caution — Level-3 hits can be weeks stale.** WebSearch is fed by a search index that lags the live board, so a result can describe a posting that has already closed. Unlike the real-time ATS responses in Level 2, a Level-3 snippet is never proof a role is still open. Add it anyway and let the evaluation fetch settle it: liveness checking was removed 2026-08-04 because its false "expired" verdicts cost real keepers, and a stale hit costs only one fetch.
